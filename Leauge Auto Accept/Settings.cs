...
namespace Leauge_Auto_Accept
{
    internal class Settings
    {
        ...
        public static bool autoStatsSkip = false;
        public static bool autoHonor = false;
        public static bool autoHonorTeammatesOrSkip = false;
        ...
        public static void SettingsModify(int item)
        {
            switch (item)
            {
                ...
                case 9:
                    ...
                    break;
                case 10:
                    autoHonorTeammatesOrSkip = !autoHonorTeammatesOrSkip;
                    break;
                case 11:
                    autoStatsSkip = !autoStatsSkip;
                    break;
                case 12:
                    UI.DelayMenu();
                    break;
            ...
            if (saveSettings) SettingsSave();
            else if (item == 0) DeleteSettings();
        }
        ...
        public static int DelayCalculateNewValue(int oldValue, int modifier)
        {
            ...
            if (modifier >= 0)
            {
                ...
                if (newNumString.Length > 9) newNumString = "999999999";
            }
            else
            {
                newNumString = newNumString.Substring(0, newNumString.Length - 1);
                if (newNumString.Length == 0) newNumString = "0"; 
            }
        ...
        public static void SaveSelectedChamp()
        {
            ...
            if (UI.currentChampPicker == 1 && "none".Contains(Navigation.currentInput.ToLower())) champsFiltered.Add(new ItemList() { Name = "None", Id = "-1" }); 
            foreach (var champ in Data.champsSorterd)
            {
                if (champ.Name.ToLower().Contains(Navigation.currentInput.ToLower()))
                {
                    if (UI.currentChampPicker == 0 && !champ.Free) continue;
        ...
        {
            if (chatMessages.Count > UI.messageIndex) chatMessages[UI.messageIndex] = Navigation.currentInput; 
            else chatMessages.Add(Navigation.currentInput); 
        ...
        {
            if (chatMessages.Count > UI.messageIndex) chatMessages.RemoveAt(UI.messageIndex);
            UpdateChatMessagesToggle();
            if (saveSettings) SettingsSave();
        }
        ...
        {
            if (chatMessages.Count > 0) chatMessagesEnabled = true;
            else chatMessagesEnabled = false;
        }
        ...
        {
            if (MainLogic.gameState.Flags.IsAutoAcceptOn)
            {
            ...
            {
                MainLogic.gameState.Flags.IsAutoAcceptOn = true;
        ...
        {
            string dirParameter = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\Leauge Auto Accept Config.txt";
            if(!File.Exists(dirParameter)) return;
            ...
            foreach (var comma in commas)
            {
                ...
                switch (columns[0])
                {
                    ...
                    case "cancelQueueAfterDodge":
                        ...
                        break;
                    case "autoHonor":
                        autoHonor = Boolean.Parse(columns[1]);
                        break;
                    case "autoHonorTeammatesOrSkip":
                        autoHonorTeammatesOrSkip = Boolean.Parse(columns[1]);
                        break;
                    case "autoStatsSkip":
                        autoStatsSkip = Boolean.Parse(columns[1]);
                        break;
                    case "chatMessages":
